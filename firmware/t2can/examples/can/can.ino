...
void setup()
{
    ...
    if (Can_A.begin(CAN20_500KBPS, MCP2518FD_40MHz) != CAN_OK)
    {
    ...
    // Longan library bug: begin() always sets the FD-format flag because it
    // compares the translated bitrate value against an enum constant. Force
    // classic CAN framing so a classic controller (Jhoinrch) can decode us.
    Can_A.__flgFDF = 0;

    Serial.println("can a fd speed: 500kbps");
#else
#error "no macro definition is set"
#endif
...
#elif defined T_2Can_Fd
            Serial.printf("can A: send data\n");
            Can_A.sendMsgBuf(0xAA, 0, CANFD::len2dlc(8), Can_Send_Package);
#else
#error "no macro definition is set"
#endif
        }
